...
pub fn get_randomizable_chests_for_area(n: &Neutopia, area_index: usize) -> Vec<Chest> {
...
pub fn write_new_chests_for_area(
    n: &Neutopia,
    area_index: usize,
    data: &mut [u8],
...
pub fn randomize_rom<R>(buffer: &mut Vec<u8>, rng: &mut R) -> Result<(), Error>
where
    R: Rng + ?Sized,
{
    ...
    // shuffle!
    randomizable_chests.shuffle(rng);

    // Have each area pick chests from the heap one by one and pass the remaining to the next area
    for i in 0..=0xf {
        write_new_chests_for_area(&n, i, buffer, &mut randomizable_chests)?;
    }
